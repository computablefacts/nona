...
public class ConcatLists extends Function {
  ...
  public ConcatLists() {
    super(eCategory.LIST_OPERATORS, "CONCAT_LISTS",
        "CONCAT_LISTS(x, y) returns the concatenation of lists x and y.");
  }
  ...
  public BoxedType<?> evaluate(List<BoxedType<?>> parameters) {
    ...
    Preconditions.checkArgument(parameters.size() == 2,
        "CONCAT_LISTS takes exactly two parameters.");
    Preconditions.checkArgument(parameters.get(0).isCollection(), "%s should be a collection",
        parameters.get(0));
    Preconditions.checkArgument(parameters.get(1).isCollection(), "%s should be a collection",
        parameters.get(1));
