...
public class SnowballProgram implements Serializable {
  ...
  protected void slice_check() {
    if (bra < 0 || bra > ket || ket > limit || limit > current.length()) // this line could be
                                                                         // removed
    {
   ...
   * 0) printf("%3d (line %4d): [%d]'", number, line_count,limit); for (i = 0; i <= limit; i++) { if
   * (z->lb == i) printf("{"); if (z->bra == i) printf("["); if (z->c == i) printf("|"); if (z->ket
   * == i) printf("]"); if (z->l == i) printf("}"); if (i < limit) { int ch = z->p[i]; if (ch == 0)
